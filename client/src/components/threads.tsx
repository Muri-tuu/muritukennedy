...
export default function Threads({ color=[1,1,1], amplitude=1, distance=0, className, style }: Props){
  ...
  useEffect(() => {
    ...
    const gl = renderer.gl as any; gl.clearColor(0,0,0,0); gl.enable(gl.BLEND); gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
    ...
    return () => { if (rafRef.current) cancelAnimationFrame(rafRef.current); window.removeEventListener('resize', resize); if (container.contains(gl.canvas)) container.removeChild(gl.canvas); (gl.getExtension('WEBGL_lose_context')?.loseContext?.()); };
  }, [amplitude, distance, color[0], color[1], color[2]]);
